...
ALTER TABLE HUE.ARTIST
	ADD COLUMN ARTIST_IMG_PRESENT_IN BOOLEAN GENERATED ALWAYS AS (
		ARTIST_IMG_BYTES IS NOT NULL AND 
		length(ARTIST_IMG_BYTES) > 0) STORED;

ALTER TABLE HUE."CHAR"
	ADD COLUMN CHAR_IMG_PRESENT_IN BOOLEAN GENERATED ALWAYS AS (
		CHAR_IMG_BYTES IS NOT NULL AND 
		length(CHAR_IMG_BYTES) > 0) STORED;

ALTER TABLE HUE.COMM
	ADD COLUMN COMM_HEADER_IMG_PRESENT_IN BOOLEAN GENERATED ALWAYS AS (
		COMM_HEADER_IMG_BYTES IS NOT NULL AND 
		length(COMM_HEADER_IMG_BYTES) > 0) STORED;
